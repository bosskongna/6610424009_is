...
def main():
    ...
    if query_type == "User Data":
        ...
        if log_query_type == "All Logs":
            ...
            results = query_logs(bucket, {"reading_type": reading_type})
        elif log_query_type == "Filter by User ID":
            user_id = st.text_input("Enter User ID")
            if user_id:
                results = query_logs(bucket, {"user_id": user_id})
            else:
                results = []
            ...
            if date:
                ...
                results = query_logs(bucket, {"reading_timestamp": date_str})
                results = [
